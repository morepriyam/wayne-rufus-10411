 ...
 *
 * Both paths start from the same positions as Shoot-and-Climb Left/Right, back
 * up at an angle,
 * and end with the robot's shooter (rear) already roughly facing the hub so
 * aim-and-shoot
 * converges quickly.
 *
 ...
 * 4. Set end waypoint to kLeftEnd / kRightEnd below (heading already computed
 * here)
 * 5. Generate → Save → the .traj file will appear in src/main/deploy/choreo/
 * 6. Deploy — the routine will then work at runtime.
 *
 * Until the .traj files exist, selecting either routine will show a Choreo
 * alert error
